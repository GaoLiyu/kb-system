...
def suggest_cases(req: SuggestCasesRequest, auth: bool = Depends(verify_token)):
    """
    根据估价对象推荐可比实例
...
def get_reference(report_type: str, auth: bool = Depends(verify_token)):
    """
    获取指定类型报告的参考数据
...
def validate_input(req: ValidateInputRequest, auth: bool = Depends(verify_token)):
    """
    验证生成输入是否完整
    """
    import sys
...
def get_input_schema(auth: bool = Depends(verify_token)):
    """
    获取输入表单字段定义
